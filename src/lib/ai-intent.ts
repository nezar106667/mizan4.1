...
export function resetChatContext() {
  setContext({ currentCustomer: null, currentMeterNumber: null, currentBill: null, currentReading: null, currentReport: null, lastDisambiguation: null, currentRanking: null, currentTimeRange: null, pendingIntent: null });
}
...
function resolveCustomer(query: string, requireContext = true, intent?: string): { customer: Customer } | { disambiguation: AiResponse } | { none: AiResponse } {
  ...
  if (result.kind === "multiple") {
    ...
    if (intent) setContext({ pendingIntent: intent });
    ...
    return { disambiguation: { kind: "disambiguation", text: `وجدت ${result.customers.length} مشتركين مطابقين — اختر المناسب بالضغط عليه:`, customers: customersData } };
  }
...
function resumeIntent(customer: Customer, intent: string | null): AiResponse {
  const s = useStore.getState();
  const meter = s.meters.find((m) => m.customer_id === customer.id);

  if (!intent || intent === "account_statement" || intent === "subscriber_ledger") {
    setContext({ currentReport: "account_statement" });
    return buildAccountStatement(customer);
  }
  if (intent === "balance") {
    const balance = customer.balance ?? 0;
    return { kind: "text", text: `المتبقي على ${customer.name}${meter ? ` (${meter.number})` : ""}: ${fmtYER(balance)}`, suggestions: ["اعرض حساب المشترك", "متى آخر قراءة؟", "هل دفع آخر فاتورة؟"] };
  }
  if (intent === "last_reading") {
    const readings = getCustomerReadings(customer.id);
    if (readings.length === 0) return { kind: "text", text: `لا توجد قراءات مسجلة لـ ${customer.name}.` };
    const r = readings[0]; setContext({ currentReading: r });
    return { kind: "text", text: `آخر قراءة لـ ${customer.name}${meter ? ` (${meter.number})` : ""}:\nالتاريخ: ${new Date(r.date).toLocaleDateString("ar-EG")}\nالقراءة: ${fmtNum(r.current)}\nالاستهلاك: ${fmtNum(r.consumption)} م³`, suggestions: ["كم المتبقي عليه؟", "كشف حساب المشترك", "هل دفع آخر فاتورة؟"] };
  }
  if (intent === "last_bill") {
    const bills = getCustomerBills(customer.id);
    if (bills.length === 0) return { kind: "text", text: `لا توجد فواتير لـ ${customer.name}.` };
    const lastBill = bills[bills.length - 1]; const r = s.readings.find((x) => x.id === lastBill.reading_id); setContext({ currentBill: lastBill });
    return { kind: "text", text: `آخر فاتورة لـ ${customer.name}:\nرقم الفاتورة: ${lastBill.serial}\nالتاريخ: ${new Date(lastBill.date).toLocaleDateString("ar-EG")}\nالاستهلاك: ${fmtNum(r?.consumption ?? 0)} م³\nالمبلغ: ${fmtYER(lastBill.total)}\nالحالة: ${lastBill.status === "paid" ? "مدفوعة" : lastBill.status === "partial" ? "جزئية" : "غير مدفوعة"}`, suggestions: ["كم المتبقي عليه؟", "كشف حساب المشترك", "آخر دفعة"] };
  }
  if (intent === "last_payment") {
    const bills = getCustomerBills(customer.id);
    const payments = getCustomerPayments(customer.id, bills).filter((p) => p.status === "approved").sort((a, b) => +new Date(b.date) - +new Date(a.date));
    if (payments.length === 0) return { kind: "text", text: `لا توجد دفعات معتمدة لـ ${customer.name}.` };
    const p = payments[0];
    return { kind: "text", text: `آخر دفعة لـ ${customer.name}:\nالتاريخ: ${new Date(p.date).toLocaleDateString("ar-EG")}\nالمبلغ: ${fmtYER(p.amount)}\nالطريقة: ${p.method === "cash" ? "نقدي" : p.method === "wallet" ? "الكريمي" : "تحويل"}`, suggestions: ["كم المتبقي عليه؟", "كشف حساب المشترك", "آخر فاتورة"] };
  }
  if (intent === "readings_history") {
    const stmt = buildAccountStatement(customer);
    if (stmt.kind === "account_statement") return { kind: "text", text: `سجل قراءات ${customer.name} (${stmt.readings.length} قراءة):\n${stmt.readings.slice(0, 10).map((r) => `${new Date(r.date).toLocaleDateString("ar-EG")}: قراءة ${fmtNum(r.current)} — استهلاك ${fmtNum(r.consumption)} م³`).join("\n")}`, suggestions: ["كشف حساب المشترك", "كم المتبقي عليه؟", "آخر فاتورة"] };
    return { kind: "text", text: `لا توجد قراءات لـ ${customer.name}.` };
  }
  if (intent === "comparison") {
    const stmt = buildAccountStatement(customer);
    if (stmt.kind === "account_statement" && stmt.monthlyConsumption.length > 0) return { kind: "text", text: `مقارنة استهلاك ${customer.name} — آخر ${stmt.monthlyConsumption.length} شهر:\n${stmt.monthlyConsumption.map((m) => `${m.month}: ${fmtNum(m.consumption)} م³`).join("\n")}`, suggestions: ["كشف حساب المشترك", "آخر قراءة؟", "طباعة كشف الحساب"] };
    return { kind: "text", text: `لا توجد بيانات كافية لمقارنة استهلاك ${customer.name}.` };
  }
  if (intent === "paid_check") {
    const bills = getCustomerBills(customer.id);
    if (bills.length === 0) return { kind: "text", text: `لا توجد فواتير لـ ${customer.name}.` };
    const lastBill = bills[bills.length - 1];
    const answer = lastBill.status === "paid" ? `نعم — ${customer.name} سدد آخر فاتورة (${lastBill.serial}) بالكامل.` : lastBill.status === "partial" ? `سدد جزئياً — المتبقي على آخر فاتورة (${lastBill.serial}): ${fmtYER(billBalance(lastBill, s.payments))}` : `لا — آخر فاتورة (${lastBill.serial}) غير مدفوعة. المبلغ المطلوب: ${fmtYER(billBalance(lastBill, s.payments))}`;
...
function noContextDisambiguation(prompt: string, intent: string): AiResponse {
  const s = useStore.getState();
  if (s.customers.length === 0) return { kind: "text", text: "لا يوجد مشتركون مسجلون في النظام." };
  const top = [...s.customers].sort((a, b) => (b.balance ?? 0) - (a.balance ?? 0)).slice(0, 8);
  setDisambiguation(top);
  setContext({ pendingIntent: intent });
  const customersData = top.map((c) => { const m = s.meters.find((x) => x.customer_id === c.id); return { id: c.id, name: c.name, meterNumber: m?.number, phone: c.phone, directorate: c.directorate, status: c.status, balance: c.balance ?? 0 }; });
  return { kind: "disambiguation", text: prompt, customers: customersData };
}
...
function answerQuestionSingle(text: string): AiResponse {
  ...
  if (numMatch && ctx.lastDisambiguation) { const idx = parseInt(numMatch[1], 10) - 1; if (idx >= 0 && idx < ctx.lastDisambiguation.length) { const c = ctx.lastDisambiguation[idx]; const pi = ctx.pendingIntent; setContext({ lastDisambiguation: null, pendingIntent: null }); setCustomerContext(c); return resumeIntent(c, pi); } }
  if (has("اول واحد", "الأول", "الاول") && ctx.lastDisambiguation && ctx.lastDisambiguation.length > 0) { const c = ctx.lastDisambiguation[0]; const pi = ctx.pendingIntent; setContext({ lastDisambiguation: null, pendingIntent: null }); setCustomerContext(c); return resumeIntent(c, pi); }
  if (has("الثاني", "ثاني واحد") && ctx.lastDisambiguation && ctx.lastDisambiguation.length > 1) { const c = ctx.lastDisambiguation[1]; const pi = ctx.pendingIntent; setContext({ lastDisambiguation: null, pendingIntent: null }); setCustomerContext(c); return resumeIntent(c, pi); }
  if (has("الثالث", "ثالث واحد") && ctx.lastDisambiguation && ctx.lastDisambiguation.length > 2) { const c = ctx.lastDisambiguation[2]; const pi = ctx.pendingIntent; setContext({ lastDisambiguation: null, pendingIntent: null }); setCustomerContext(c); return resumeIntent(c, pi); }
  ...
  if (has("كم عليه", "كم باقي", "كم المتأخر", "كم المطلوب", "رصيده", "ديونه", "كم المتبقي", "كم المتبقى", "كم بقي", "كم تبقى", "كم مستحق", "هل عليه", "المطلوب", "المستحق", "المديونية", "مديونية", "كم ديونه", "كم رصيده", "كم عليه دين", "دين")) {
    ...
    if (meterToken) resolved = resolveCustomer(meterToken, false, "balance"); else { const hasName = s.customers.some((c) => text.includes(c.name.split(/\s+/)[0])); resolved = hasName ? resolveCustomer(text, false, "balance") : { none: { kind: "suggestions", text: "", suggestions: [] } }; }
    ...
    return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "balance");
  }
  ...
  if (has("آخر قراءة", "اخر قراءة", "آخر استهلاك", "اخر استهلاك", "متى آخر قراءة", "متى اخر قراءة", "اخر الاستهلاك", "الاستهلاك الاخير")) {
    if (!ctx.currentCustomer) return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "last_reading");
  ...
  if (has("آخر فاتورة", "اخر فاتورة", "متى آخر فاتورة", "متى اخر فاتورة", "كم استهلك هذا الشهر", "كم استهلك")) {
    if (!ctx.currentCustomer) return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "last_bill");
  ...
  if (has("سجل القراءات", "قراءات", "كل القراءات", "قائمة القراءات", "عرض القراءات", "تاريخ القراءات")) {
    if (!ctx.currentCustomer) return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "readings_history");
  ...
  if (has("مقارنة", "قارن", "آخر 12 شهر", "12 شهر", "استهلاك السنة", "مقارنة استهلاك", "قارن استهلاكه", "قارن بالشهر الماضي", "مقارنة بالشهر الماضي", "مقارنة الشهور")) {
    if (!ctx.currentCustomer) return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "comparison");
    const stmt = buildAccountStatement(ctx.currentCustomer);
  ...
  if (has("آخر دفعة", "اخر دفعة", "آخر تحصيل", "اخر تحصيل", "متى دفع", "متى آخر دفعة", "دفع آخر", "متى دفع آخر مرة")) {
    if (!ctx.currentCustomer) return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "last_payment");
    const bills = getCustomerBills(ctx.currentCustomer.id);
  ...
  if (has("هل دفع", "هل سدد", "هل دفع آخر فاتورة", "هل سدد آخر فاتورة", "هل دفع الفاتورة")) {
    if (!ctx.currentCustomer) return noContextDisambiguation("أي مشترك تقصد؟ اختر بالضغط:", "paid_check");
    const bills = getCustomerBills(ctx.currentCustomer.id);
    if (bills.length === 0) return { kind: "text", text: `لا توجد فواتير لـ ${ctx.currentCustomer.name}.` };
    const lastBill = bills[bills.length - 1];
  ...
  if (has("كشف حساب", "اعرض حساب", "كشف الحساب", "بيان حساب", "تفاصيل المشترك", "اعرض تفاصيل")) {
    const resolved = resolveCustomer(text, true, "account_statement");
    if ("customer" in resolved) { const meter = s.meters.find((m) => m.customer_id === resolved.customer.id); setCustomerContext(resolved.customer, meter?.number); setContext({ currentReport: "account_statement" }); return buildAccountStatement(resolved.customer); }
    if ("disambiguation" in resolved) return resolved.disambiguation;
  ...
  if (has("استعلام عن مشترك", "مشترك", "حساب", "كشف حساب", "ذمة", "رصيد", "فواتير المشترك", "اعرض فواتير")) {
    const resolved = resolveCustomer(text, true, "subscriber_ledger");
  ...
  const topC = [...s2.customers].sort((a, b) => (b.balance ?? 0) - (a.balance ?? 0))[0];
  const topMeter = topC ? s2.meters.find((m) => m.customer_id === topC.id) : null;
  const sampleQuery = topC && topMeter ? `كشف حساب المشترك ${topMeter.number}` : "كشف حساب مشترك";
  return { kind: "suggestions", text: "اختر استعلاماً — ميزان الذكي يدعم هذه التقارير:", suggestions: ["استعلام عن مشترك", sampleQuery, "تحليل الفاقد لهذا الشهر", "من دفع ومن لم يدفع؟", "استعلام عن التحصيل اليوم", "كم عدد المشتركين؟", "كم إجمالي الديون؟", "ما أكثر المشتركين تأخراً؟", "أعلى مستهلك", "سجل القراءات"] };
}
