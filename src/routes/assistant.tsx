...
function dynamicSuggestions(): string[] {
  const s = useStore.getState();
  const top = [...s.customers]
    .filter((c) => (c.balance ?? 0) > 0)
    .sort((a, b) => (b.balance ?? 0) - (a.balance ?? 0))[0];
  const meter = top ? s.meters.find((m: Meter) => m.customer_id === top.id) : null;
  return [
    "استعلام عن مشترك",
    top && meter ? `كشف حساب المشترك ${meter.number}` : "كشف حساب المشترك",
