...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const tenantFilter = `tenant_id=eq.${user.tenantId}`;
    const channel = supabase
      .channel("mizan-live")
      .on("postgres_changes", { event: "*", schema: "public", table: "customers", filter: tenantFilter }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "water_readings", filter: tenantFilter }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "water_bills", filter: tenantFilter }, refresh)
      .on("postgres_changes", { event: "*", schema: "public", table: "payments", filter: tenantFilter }, refresh)
      .subscribe();
