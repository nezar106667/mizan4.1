...
import { exportStatementPDF, exportLossPDF, exportRevenuePDF, exportPaymentStatusPDF } from "@/lib/assistant/pdf-export";
...
export function AiResponseRenderer({ response, onSuggestion }: Props) {
  ...
  if (response.kind === "revenue_report") {
    return (<Card><CardContent className="p-4 space-y-3"><div className="text-xs text-muted-foreground">{response.range.label}: {response.range.from} → {response.range.to}</div>
      ...
      <div className="flex gap-1.5 flex-wrap pt-1"><Button size="sm" variant="outline" onClick={() => exportRevenuePDF(response)}><Printer className="w-3 h-3 ms-1" /> طباعة PDF</Button></div>
